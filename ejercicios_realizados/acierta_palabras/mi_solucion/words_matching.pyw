...
class WordsMatchingWidget(QtWidgets.QWidget):
    """
    ...
    def __init__(self,
                 ...
                 n_words=5,
                 letters_to_show=4,
                 min_letters_per_word=8,
                 max_letters_per_word=8,
                 parent=None):
        ...
        self.min_letters_per_word = min_letters_per_word
        self.max_letters_per_word = max_letters_per_word
        self.letters_to_show = letters_to_show
        self.n_words = n_words
        self.words = list()
        self.words_definitions = list()
    ...
    def _checkAnswer(self):
        # check if answer is in format 'number:str'
        pattern = '\d:[a-zA-Z]*[^:]'
        _rex = re.compile(pattern)
        answer = self.ui.lineEdit.text()
        if _rex.fullmatch(answer):
            logging.debug("WordsMatchingWidget._checkAnswer: answer=%s.", answer)
            l = answer.split(':')
            answer_index = int(l[0]) - 1
            answer_word = l[1].upper()
            try:
                correct_answer_word = self.words[answer_index].upper()
            except IndexError as e:
                return
            logging.info("WordsMatchingWidget._checkAnswer: answer_word=%s, correct_word=%s.",
                         answer_word, correct_answer_word)
            if correct_answer_word == answer_word:
                self.successes = self.successes + 1
                del self.words[answer_index]
                del self.words_definitions[answer_index]
                self.showWords()
                self.ui.wordDefinitionTextEdit.clear()
...
if __name__ == "__main__":
    ...
    myapp = WordsMatchingWidget(play_time,
                                interval,
                                n_words,
                                letters_to_show,
                                min_n_letters_per_word,
                                max_n_letters_per_word,
                                None)
    myapp.show()
    sys.exit(app.exec_())
